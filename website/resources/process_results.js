const extensionFromParty = {
    "bdp": "gif", "cvp": "svg", "eco": "jpg", "edu": "svg", "evp": "png", "fdp": "svg", "fps": "png",
...
function generateBlocPartyRecomm(voteInfo, lang) {
    ...
    const recom2color = { 1: "text-green-700", 2: "text-red-700", 4: "text-gray-700", 5: "text-slate-300", 8: "text-red-700", 9: "text-green-700", 66: "text-gray-700" };
    const recom2label = {
    ...
    const recom2icon = {
        ...
        2: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        4: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
      <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
    </svg>`,
        5: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
  </svg>`,
        66: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
</svg>`,
    };
...
<span class="flex-1 flex gap-1"><span class="${recom2color[cfPos]}">${recom2icon[cfPos]}</span> ${recom2label[lang][cfPos]}</span>
</div>`);
    addInnerHTML(partyRecommBloc, `<div class="border-black flex py-4 items-center gap-6" title="${cnName}">
<img src="resources/party_logos/swiss_council.svg" class="m-auto w-20 h-10 object-contain" />
<span class="flex-1 flex gap-1"><span class="${recom2color[cnPos]}">${recom2icon[cnPos]}</span> ${recom2label[lang][cnPos]}</span>
</div>`);
    ...
    for (const partyAcronym of listParties) {
        if (partyAcronym in extensionFromParty) {
            ...
            if (partyRecommendation in recom2color) {
                addInnerHTML(partyRecommBloc, `<div class="border-black flex py-4 items-center gap-6" title="${nameFromAcronym[lang][partyAcronym]} (${recom2label[lang][partyRecommendation]})">
<img src="resources/party_logos/${partyAcronym}.${extensionFromParty[partyAcronym]}" class="m-auto w-20 h-10 object-contain" />
<span class="flex-1 flex gap-1"><span class="${recom2color[partyRecommendation]}">${recom2icon[partyRecommendation]}</span> ${recom2label[lang][partyRecommendation]}</span>
</div>`);
            }
        }
    }
